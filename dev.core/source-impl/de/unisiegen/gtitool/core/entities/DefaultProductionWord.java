...
public final class DefaultProductionWord implements ProductionWord
{
  ...
  public DefaultProductionWord ( Element element ) throws StoreException
  {
    ...
    if ( !element.getName ().equals ( "ProductionWord" ) )
    ...
    for ( Element current : element.getElement () )
      ...
      else if ( current.getName ().equals ( "NonterminalSymbol" ) )
   ...
   * {@inheritDoc}
   */
  public final boolean contains(final NonterminalSymbol ns)
  {
    if(ns == null)
      throw new NullPointerException("symbol is null"); //$NON-NLS-1$
    for(ProductionWordMember dpwm : this.productionWordMemberList)
      if(dpwm.getName ().equals ( ns.getName () ))
        return true;
    return false;
  }
   ...
   * {@inheritDoc}
   */
  public final boolean contains(final TerminalSymbol ts)
  {
    if(ts == null)
      throw new NullPointerException("symbol is null"); //$NON-NLS-1$
    for(ProductionWordMember dpwm : this.productionWordMemberList)
      if(dpwm.getName ().equals ( ts.getName () ))
        return true;
    return false;
  }
}
