...
final public class DFA
{
  ...
  /**
   * <code>isPushback[state] == true</code> <=> the state <code>state</code>
   * is a final state of an expression that can only be matched when followed by
   * a certain lookaead.
   */
  boolean [] isPushback;
  ...
  /**
   * <code>isLookEnd[state] == true</code> <=> the state <code>state</code>
   * is a final state of a lookahead expression.
   */
  boolean [] isLookEnd;
  ...
  public void checkActions ( LexScan scanner, LexParse parser )
  {
    ...
    while ( l.hasMoreElements () )
    {
      ...
      if ( !next.equals ( usedActions.get ( next ) )
          && !eofActions.isEOFAction ( next ) )
        Out.warning ( scanner.file, ErrorMessages.NEVER_MATCH,
            ( ( Action ) next ).priority - 1, -1 );
    }
  }
  ...
  public void minimize ()
  {
     ...
     * (block[i+1] == block[j+1])) { System.out.println("error:
     * equiv["+i+"]["+j+"] = "+equiv[i][j]+ ", block["+i+"] = "+block[i+1]+",
     * block["+j+"] = "+block[j]); error = true; } } } if (error)
     * System.exit(1); System.out.println("check");
     */
    ...
    for ( int i = n ; i <= last ; i++ )
    {
      ...
      while ( s != i )
      {
        ...
        if ( s != i )
        {
          line = line + ",";
          if ( b [ s ] != i )
            Out.dump ( "consistency error for state " + ( s - 1 ) + " (block "
                + b [ s ] + ")" );
        }
        if ( b_b [ s ] != t )
    ...
    for ( i = 1 ; i < numStates ; i++ )
    {
      ...
      for ( j = 0 ; j < i ; j++ )
      {
        ...
        if ( isFinal [ i ] && isFinal [ j ]
            && ( isPushback [ i ] == isPushback [ j ] )
            && ( isLookEnd [ i ] == isLookEnd [ j ] ) )
