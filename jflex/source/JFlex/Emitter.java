...
final public class Emitter
{
  ...
  // table[i][j] is the set of input characters that leads from state i to state
  // j
  ...
  private void emitMain ()
  {
    ...
    println ( "      System.out.println(\"Usage : java " + scanner.className
        + " <inputfile>\");" );
    println ( "    }" );
    println ( "    else {" );
    ...
    println ( "        try {" );
    println ( "          scanner = new " + scanner.className
  ...
  private void emitClassName ()
  {
    ...
    println ( "@ SuppressWarnings ( value = { \"all\" } )" );
    if ( scanner.isPublic )
      print ( "public " );
  ...
  private void emitLexicalStates ()
  {
    ...
    while ( stateNames.hasMoreElements () )
    {
      ...
      if ( scanner.bolUsed )
        ...
        println ( "  " + visibility + " static final int " + name + " = "
            + dfa.lexState [ 2 * num ] + ";" );
    }
  ...
  private void emitGetRowMapNext ()
  {
    ...
    if ( scanner.lookAheadUsed )
    {
      println ( "          if ( (zzAttributes & " + PUSHBACK + ") == "
          + PUSHBACK + " )" );
    ...
    println ( "          if ( (zzAttributes & " + FINAL + ") == " + FINAL
        + " ) {" );
    if ( scanner.lookAheadUsed )
      println ( "            zzWasPushback = (zzAttributes & " + LOOKEND
          + ") == " + LOOKEND + ";" );
    ...
    if ( eofActions.numActions () > 0 )
    {
      ...
      while ( stateNames.hasMoreElements () )
      {
        ...
        if ( !scanner.bolUsed )
        {
          ...
          if ( !unused )
            Out.warning ( "Lexical states <" + name + "> and <"
                + used.get ( key ) + "> are equivalent." );
          else
            used.put ( key, name );
        }
