...
final public class Macros
{
  ...
  public boolean insert ( String name, RegExp definition )
  {
    ...
    if ( Options.DEBUG )
      Out
          .debug ( "inserting macro " + name + " with definition :" + Out.NL + definition ); //$NON-NLS-1$ //$NON-NLS-2$
   ...
   * @return the definition of the macro, <code>null</code> if no macro with
   *         the specified name has been stored.
   ...
   * @throws MacroException when an error (such as a cyclic definition) occurs
   *           during expansion
   */
    ...
    switch ( definition.type )
    {
      ...
      case sym.MACROUSE :
        ...
        if ( name.equals ( usename ) )
          throw new MacroException ( ErrorMessages.get (
              ErrorMessages.MACRO_CYCLE, name ) );

        RegExp usedef = getDefinition ( usename );

        if ( usedef == null )
          throw new MacroException ( ErrorMessages.get (
              ErrorMessages.MACRO_DEF_MISSING, usename, name ) );
      ...
      default :
        throw new MacroException (
            "unknown expression type " + definition.type + " in macro expansion" ); //$NON-NLS-1$ //$NON-NLS-2$
    }
  }
}
