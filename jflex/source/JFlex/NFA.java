...
final public class NFA
{
  ...
  public NFA ( int numInput, LexScan scanner, RegExps regExps, Macros macros,
      CharClasses classes )
  {
  ...
  public void addStandaloneRule ()
  {
    ...
    action [ end ] = new Action ( "System.out.print(yytext());",
        Integer.MAX_VALUE );
    isFinal [ end ] = true;
  }
  ...
  public void addRegExp ( int regExpNum )
  {
    ...
    if ( Options.DEBUG )
      Out.debug ( "Adding nfa for regexp " + regExpNum + " :" + Out.NL
          + regExps.getRegExp ( regExpNum ) );
  ...
  private boolean containsPushback ( StateSet set )
  {
    ...
    while ( states.hasMoreElements () )
      if ( isPushback [ states.nextElement () ] )
        return true;
  ...
  {
    // Out.debug("Calculating DFAEdge for state set "+start+" and input
    // '"+input+"'");
  ...
  public DFA getDFA ()
  {
    ...
    if ( Options.DEBUG )
      Out.debug ( "DFA start states are :" + Out.NL + dfaStates + Out.NL
          + Out.NL + "ordered :" + Out.NL + dfaVector );
  ...
  private void insertLetterNFA ( boolean caseless, char letter, int start,
      int end )
  {
    if ( caseless )
    {
    ...
    for ( i = 0 ; i < letters.length () ; i++ )
    {
      ...
      else
      {
        addTransition ( i + start,
            classes.getClassCode ( letters.charAt ( i ) ), i + start + 1 );
      }
    }
  ...
  private IntPair complement ( IntPair nfa )
  {
    ...
    if ( Options.DEBUG )
      Out.debug ( "pos DFA start state is :" + Out.NL + dfaStates + Out.NL
          + Out.NL + "ordered :" + Out.NL + dfaVector );
    ...
    while ( currentDFAState <= numDFAStates )
    {
      ...
      for ( char input = 0 ; input < numInput ; input++ )
      {
        ...
        if ( newState.containsElements () )
        {
          ...
          {
            // Out.debug("FOUND!");
            addTransition ( dfaStart + currentDFAState, input, dfaStart
                + nextDFAState.intValue () );
          }
          else
          {
            if ( Options.dump )
    ...
    if ( Options.DEBUG )
      Out.debug ( "complement finished, nfa (" + start + "," + end
          + ") is now :" + this );
  ...
  private void insertNFA ( RegExp regExp, int start, int end )
  {
    switch ( regExp.type )
    {
      ...
      case sym.CCLASSNOT :
        insertNotClassNFA ( ( Vector ) ( ( RegExp1 ) regExp ).content, start,
            end );
        return;

      case sym.CHAR :
        insertLetterNFA ( false,
            ( ( Character ) ( ( RegExp1 ) regExp ).content ).charValue (),
            start, end );
        return;

      case sym.CHAR_I :
        insertLetterNFA ( true, ( ( Character ) ( ( RegExp1 ) regExp ).content )
            .charValue (), start, end );
        return;

      case sym.MACROUSE :
        insertNFA ( macros
            .getDefinition ( ( String ) ( ( RegExp1 ) regExp ).content ),
            start, end );
        return;
    }

    throw new Error ( "Unknown expression type " + regExp.type
        + " in NFA construction" );
  }
  ...
  public IntPair insertNFA ( RegExp regExp )
  {
    ...
    switch ( regExp.type )
    {
      ...
      case sym.STRING :
        return insertStringNFA ( false,
            ( String ) ( ( RegExp1 ) regExp ).content );

      case sym.STRING_I :
        return insertStringNFA ( true,
            ( String ) ( ( RegExp1 ) regExp ).content );

      case sym.MACROUSE :
        return insertNFA ( macros
            .getDefinition ( ( String ) ( ( RegExp1 ) regExp ).content ) );
    }
