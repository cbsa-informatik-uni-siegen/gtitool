...
/**
 * Performs simple semantic analysis on regular expressions. (used for checking
...
public final class SemCheck
{
  ...
  /**
   * Performs semantic analysis for all expressions. Currently: illegal
   * lookahead check only [fixme: more checks possible]
   * 
  ...
  /**
   * Checks for illegal lookahead expressions. Lookahead in JFlex only works
   * when the first expression has fixed length or when the intersection of the
   * last set of the first expression and the first set of the second expression
   * is empty. FIXME: this check is much too weak
   * 
  ...
  private static int length ( RegExp re )
  {
    ...
    switch ( re.type )
    {
      ...
      case sym.MACROUSE :
        return length ( macros
            .getDefinition ( ( String ) ( ( RegExp1 ) re ).content ) );
    }
