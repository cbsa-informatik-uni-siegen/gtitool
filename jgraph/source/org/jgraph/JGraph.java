/*
 * @(#)JGraph.java Copyright (c) 2001-2006 Gaudenz Alder
 */
package org.jgraph;
    ...
    implements Scrollable, Accessible, Serializable
{
  ...
  /**
   * The buffer around the offscreen graphics object that provides the specified
   * distance of scrolling before the buffer has to be redrawn. Increasing the
   * value means fewer redraws but more memory is required.
   */
  ...
  /**
   * True if the graph allows invalid null ports during previews (aka flip back
  ...
  //
  /**
   * True if Drag-and-Drop should be used for move operations. Default is false
   * due to a JDK bug.
   */
   ...
   * in graph changing or other means <code>stopCellEditing</code> is invoked,
   * and changes are saved. If false, <code>cancelCellEditing</code> is
   * invoked, and changes are discarded.
   */
  ...
  public static void addSampleData ( GraphModel model )
  {
    ...
    GraphConstants.setDashPattern ( implementStyle, new float []
    { 3, 3 } );
    ...
    attributes.put ( cu,
        createBounds ( new AttributeMap (), 340, 20, Color.red ) );
    cu.addPort ( null, "ComponentUI/Center" );
    DefaultGraphCell gu = new DefaultGraphCell ( "GraphUI" );
    attributes.put ( gu, createBounds ( new AttributeMap (), 340, 100,
        Color.red ) );
    ...
    { jgAggregatesGm, jcAggregatesCu, modelGroup, jc, jg, jgExtendsJc, uiGroup };
  ...
  public static Map createBounds ( AttributeMap map, int x, int y, Color c )
  {
    ...
    GraphConstants.setGradientColor ( map, c.brighter ().brighter ()
        .brighter () );
  ...
  /**
   * Returns an instance of <code>JGraph</code> which displays the data model
  ...
  /**
   * Returns all <code>cells</code> including all descendants in the passed in
   * order of cells.
   */
  ...
  /**
   * Returns a map of (cell, clone)-pairs for all <code>cells</code> and their
   * children. Special care is taken to replace the anchor references between
   * ports. (Iterative implementation.)
   */
   ...
   * bounds on non-leafs to check for containment. If reverse is true this will
   * return the innermost view.
   */
   ...
   * <code>current</code> cell. Returns the topmost cell if there are no more
   * cells behind <code>current</code>. Note: This does only return visible
   * cells.
   */
   ...
   * This is used to iterate overlapping cells, and cells that are grouped. The
   * current selection affects this method. <br>
   ...
   * the specified array of views. The views must be in order, as returned, for
   ...
   * does not define an offset) or <b>the </b> port, if there is only one port.
   * 
  ...
  public PortView getDefaultPortForCell ( Object cell )
  {
    if ( cell != null && !getModel ().isEdge ( cell ) )
    {
      ...
      for ( int i = 0 ; i < childCount ; i++ )
      {
        ...
        {
          Point2D offset = GraphConstants
              .getOffset ( child.getAllAttributes () );
  ...
  public Rectangle2D toScreen ( Rectangle2D rect )
  {
    if ( rect == null )
      return null;
    rect.setFrame ( rect.getX () * scale, rect.getY () * scale, rect
        .getWidth ()
  ...
  public void updateAutoSize ( CellView view )
  {
    if ( view != null && !isEditing () )
    {
      ...
      if ( bounds != null )
      {
        boolean autosize = GraphConstants
            .isAutoSize ( view.getAllAttributes () );
  ...
  /**
   * Returns the attributes for the specified cell. If the layout cache returns
   * a view for the cell then this method returns allAttributes, otherwise the
   * method returns model.getAttributes(cell).
   */
  ...
  /**
   * Sets if selected edges should be disconnected from unselected vertices when
   * they are moved.
   */
  ...
  /**
   * Returns true if cells should be added to groups when moved over the group's
   * area.
   */
  ...
  /**
   * Sets whether or not the graph should be automatically resize when cells are
  ...
  /**
   * Determines whether the graph is editable. Fires a property change event if
   * the new setting is different from the existing setting.
   * <p>
   * Note: Editable determines whether the graph allows editing. This is not to
   * be confused with enabled, which allows the graph to handle mouse events
   * (including editing).
   * 
  ...
  /**
   * Determines whether cell selection is enabled. Fires a property change event
  ...
  /**
   * Returns the current double buffering graphics object. Checks to see if the
   * graph bounds has changed since the last time the off screen image was
  ...
  public Graphics getOffgraphics ()
  {
    ...
    int x = Math.max ( 0, ( int ) viewPortBounds.getX ()
        - ( int ) offscreenBuffer );
    int y = Math.max ( 0, ( int ) viewPortBounds.getY ()
        - ( int ) offscreenBuffer );
    int width = ( int ) viewPortBounds.getWidth () + ( int ) offscreenBuffer
        * 2;
    ...
    if ( !isOffscreenValid () )
    {
      ...
      offgraphics.fillRect ( 0, 0, ( int ) graphBounds.getWidth (),
          ( int ) graphBounds.getHeight () );
   ...
   */
  protected void setupOffScreen ( int x, int y, int width, int height,
  ...
  /**
   * @param offscreenValid the offscreenValid to set
   */
   ...
   * 
   * @param newLayoutCache the <code>GraphLayoutCache</code> that is to
   *          provide the view-data
   */
  ...
  /**
   * Sets the <code>MarqueeHandler</code> that will handle marquee selection.
   * 
  ...
  /**
   * Returns <code>true</code> if the graph and the cell are editable. This is
   * invoked from the UI before editing begins to ensure that the given cell can
   * be edited.
   * 
   ...
   * in order to allow the graph to create a tooltip for the topmost cell under
   * the mousepointer. This differs from JTree where the renderers tooltip is
   * used.
   * <p>
      ...
      if ( view != null )
      {
        ...
        if ( c instanceof JComponent )
        {
          ...
          e = new MouseEvent ( c, e.getID (), e.getWhen (), e.getModifiers (),
              ...
              ( int ) ( where.getY () - rect.getY () ), e.getClickCount (), e
                  .isPopupTrigger () );
   ...
   *          when a cell is selected or deselected (a "negative selection")
   */
  ...
  public void scrollCellToVisible ( Object cell )
  {
    ...
    if ( bounds != null )
    {
      ...
      scrollRectToVisible ( new Rectangle ( ( int ) b2.getX (), ( int ) b2
          .getY (), ( int ) b2.getWidth (), ( int ) b2.getHeight () ) );
    }
  }
  ...
  {
    if ( p != null )
      scrollRectToVisible ( new Rectangle ( ( int ) p.getX (), ( int ) p
          .getY (), 1, 1 ) );
  }
  ...
  /**
   * Ends the current editing session. (The <code>DefaultGraphCellEditor</code>
  ...
  /**
   * Cancels the current editing session. Has no effect if the graph isn't being
   * edited.
   */
  ...
  /**
   * Selects the specified cell and initiates editing. The edit-attempt fails if
   * the <code>CellEditor</code> does not allow editing for the specified
   * item.
   */
   ...
   * inserted into the graph). You should never have to invoke this, the UI will
   * invoke this as it needs to. (Note: This is invoked by GraphUI, eg. after
   * moving.)
   */
   ...
   * background is applied to the image, else the background is filled with the
   * bg color. Therefore, one should only use a null background if the
    ...
    if ( bounds != null )
    {
      ...
      if ( graphicsConfig != null )
      {
        img = getGraphicsConfiguration ().createCompatibleImage (
            ...
            ( bg != null ) ? Transparency.OPAQUE : Transparency.BITMASK );
      }
      else
      {
  ...
  //
  /**
   * Returns the preferred display size of a <code>JGraph</code>. The height
   * is determined from <code>getPreferredWidth</code>.
   * 
   ...
   * the width of the graph, unless the preferred width of the graph is smaller
   * than the viewports width. In other words: ensure that the graph is never
   * smaller than its viewport.
   * 
   * @return false
   ...
   * determine the height of the graph, unless the preferred height of the graph
   * is smaller than the viewports height. In other words: ensure that the graph
  ...
  /**
   * Returns a string representation of this <code>JGraph</code>. This method
