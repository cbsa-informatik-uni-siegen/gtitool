/*
 * @(#)GraphModelEvent.java 1.0 03-JUL-04 Copyright (c) 2001-2005 Gaudenz Alder
...
public class GraphLayoutCacheEvent extends EventObject
{
  ...
  /**
   * Used to create an event when cells have been changed, inserted, or removed,
   * identifying the change as a GraphLayoutCacheChange object.
   * 
   * @param source the Object responsible for generating the event (typically
   *          the creator of the event object passes <code>this</code> for its
   *          value)
  ...
  /**
   * Defines the interface for objects that may be used to represent a change to
  ...
  public static interface GraphLayoutCacheChange
  {
     ...
     * if this change is a GraphModelChange. Note: This is not necessarily the
     * same as the source of the event and is used separately in the
     * graphundomanager.
     * 
     ...
     * attributes for each changed cell. Note: This returns a map of (cell, map)
     * pairs for an insert on a model that is not an attribute store. Use
     * getPreviousAttributes to access the attributes that have been stored in
     * the model.
     */
    ...
    /**
     * Returns a map that contains (object, map) pairs which holds the previous
     * attributes for the changed cells.
     * 
     ...
     * because one of their dependent cells has changed. This is typically used
     * to return the edges that are attached to vertices, which in turn have
