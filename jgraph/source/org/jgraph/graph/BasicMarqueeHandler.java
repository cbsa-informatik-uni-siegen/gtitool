/*
 * @(#)BasicMarqueeHandler.java 1.0 03-JUL-04 Copyright (c) 2001-2004 Gaudenz
 * Alder
 */
...
public class BasicMarqueeHandler
{
  ...
  public void mouseReleased ( MouseEvent e )
  {
    try
    {
      if ( e != null && marqueeBounds != null )
      {
        ...
        Rectangle2D bounds = graph.fromScreen ( ( Rectangle2D ) marqueeBounds
            .clone () );
        ...
        Rectangle dirty = new Rectangle ( ( int ) marqueeBounds.getX (),
            ...
            ( int ) marqueeBounds.getWidth () + 1, ( int ) marqueeBounds
                .getHeight () + 1 );
        dirty.width++ ;
        dirty.height++ ;
        graph.repaint ( dirty );
  ...
  /**
   * Draw the current state of the handler. This is called twice by the overlay
   * method and also by the paint method. The caller's intention is that the
   * overlay method draws exactly the current state of the handler so that it
   * may be used for XOR paint. The drag method calls overlay, changes the
   * state, and calls overlay again to use this. However, since it is not always
   * possible to clear the screen with an exact repaint the caller passes a flag
   * to indicate if the graphics object should be cleared with this call (eg. if
   * a subsequent call follows).
   * 
   * @param g
   */
  public void overlay ( JGraph graph, Graphics g, boolean clear )
  {
    if ( marqueeBounds != null )
    {
      ...
      g.drawRect ( ( int ) marqueeBounds.getX (),
          ( int ) marqueeBounds.getY (), ( int ) marqueeBounds.getWidth (),
