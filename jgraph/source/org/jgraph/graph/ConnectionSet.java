/*
 * @(#)ConnectionSet.java 1.0 03-JUL-04 Copyright (c) 2001-2004 Gaudenz Alder
 */
...
public class ConnectionSet implements Serializable
{
   ...
   * <code>cells</code> in <code>model</code> based on
   ...
   * <code>target</code> in <code>cs</code> based on <code>disconnect</code>.
   */
   ...
   * or null if the connection set contains no corresponding entry for the edge.
   */
  ...
  /**
   * Creates a new connection set based on this connection set, where the edges,
   * and ports are mapped using <code>map</code>. If a port is not found, the
   * old port is used. If both, the edge and the port are not in
   * <code>map</code>, the entry is ignored.
   * <p>
   * <strong>Note: </strong> Consequently, unselected edges are only reconnected
   * at the first "paste" after a "cut", because in this case the ConnectionSet
   * is not cloned.
   */
  ...
  public static class Connection implements Serializable
  {
    ...
    /**
     * Constructs a new source or target connection between <code>edge</code>
     * and <code>port</code> based on the value of <code>source</code>
     */
    ...
    /**
     * Two connections are equal if they represent the source or target of the
     * same edge. That is, if
     * <p>
