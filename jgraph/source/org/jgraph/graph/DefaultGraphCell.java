/*
 * @(#)AbstractGraphCell.java 1.0 03-JUL-04 Copyright (c) 2001-2006 Gaudenz
 * Alder
 */
    ...
    GraphCell, Cloneable
{
  ...
  /**
   * Creates a graph cell and initializes it with the specified user object. The
   * GraphCell allows children only if specified.
   * 
   * @param userObject an Object provided by the user that constitutes the
   *          cell's data
   * @param storageMap the storage attribute map for this cell
  ...
  /**
   * Utility method to create a port for this cell. This method adds a floating
   * port.
   * 
  ...
  /**
   * Utility method to create a port for this cell. The method adds a port at a
   * fixed relative offset within the cell. If the offset is null then a
  ...
  /**
   * Create a clone of the cell. This method uses the superclass implementation
   * (which does not clone the children), then uses clone on the attribute map.
   * This method does not clone the user object. You should override the
   * cloneUserObject in the graph model to implement cloning of custom user
   * objects.
   * 
