/*
 * @(#)EdgeView.java 1.0 03-JUL-04 Copyright (c) 2001-2004 Gaudenz Alder
 */
...
public class EdgeView extends AbstractCellView
{
  ...
  /**
   * Overrides the parent method to udpate the cached points, source and target
   * port. If the source or target is removed, a point is inserted into the
   * array of points.
   */
  ...
  /**
   * Returns the local renderer. Do not access the renderer field directly. Use
   * this method instead. Note: This method is package private.
   */
  ...
  public Point2D getPoint ( int index )
  {
    ...
    if ( index == 0 && sourceParentView != null )
    {
      return sourceParentView.getPerimeterPoint ( this,
          getCenterPoint ( sourceParentView ), getNearestPoint ( index == 0 ) );
    }
    else if ( index == getPointCount () - 1 && targetParentView != null )
    {
      return targetParentView.getPerimeterPoint ( this,
          getCenterPoint ( targetParentView ), getNearestPoint ( index == 0 ) );
    }
  ...
  /**
   * Returns the nearest point wrt to the source or target. This method returns
   * the next or previous point or port in the points list, eg. if source is
   * true it returns the location of the point or port at index 1 without
   * calling the getLocation method on any ports.<br>
   ...
   * <br>
   * This is used from within getPoint to pass the nearest point to the portview
   * to find it's location. This uses the center point of the parent view to
   * determine the port view's location to avoid infinite recursion.
   */
  ...
  public void removeExtraLabel ( int index )
  {
    ...
    if ( labels == null || labels.length > 1 )
    {
      ...
      if ( index < newPts.length )
        System
            .arraycopy ( pts, index + 1, newPts, index, newPts.length - index );
  ...
  public static class EdgeHandle implements CellHandle, Serializable
  {
    ...
    public void paint ( Graphics g )
    {
      ...
      for ( int i = 0 ; i < r.length ; i++ )
      {
        ...
        g.fill3DRect ( ( int ) r [ i ].getX (), ( int ) r [ i ].getY (),
            ( int ) r [ i ].getWidth (), ( int ) r [ i ].getHeight (), true );
        CellView port = null;
        ...
        {
          g.setColor ( graph.getLockedHandleColor () );
          Point2D tmp = ( port != null ) ? GraphConstants.getOffset ( port
              .getAllAttributes () ) : null;
          if ( tmp != null )
          {
            g.drawLine ( ( int ) r [ i ].getX () + 1,
                ( int ) r [ i ].getY () + 1, ( int ) ( r [ i ].getX () + r [ i ]
                    .getWidth () ) - 3, ( int ) ( r [ i ].getY () + r [ i ]
                    .getHeight () ) - 3 );
            g.drawLine ( ( int ) r [ i ].getX () + 1,
            ...
            g.drawRect ( ( int ) r [ i ].getX () + 2,
                ( int ) r [ i ].getY () + 2, ( int ) r [ i ].getWidth () - 5,
                ( int ) r [ i ].getHeight () - 5 );
        }
      }
    ...
    protected boolean snap ( boolean source, Point2D point )
    {
      ...
      {
        CellView portView = graph.getGraphLayoutCache ().getMapping ( port,
            false );
    ...
    public void mousePressed ( MouseEvent event )
    {
      ...
      if ( !isEditing () && graph.isMoveable ()
          && GraphConstants.isMoveable ( edge.getAllAttributes () )
          && loc != null && loc.contains ( x, y ) && !isAddPointEvent ( event )
          && !isRemovePointEvent ( event ) && graph.getEdgeLabelsMovable () )
      {
          ...
          && GraphConstants.isMoveable ( edge.getAllAttributes () ) )
      {
        for ( int i = 0 ; i < extraLabelLocations.length ; i++ )
        {
          ...
          {
            currentLabel = i;
            initialLabelLocation = ( Point2D ) edge.getExtraLabelPosition (
                currentLabel ).clone ();
      ...
      else if ( isEditing () && currentPoint != null )
      {
        ...
        if ( source )
          disconnectable = disconnectable
              && ( ( orig.getSource () == null && orig.getSourceParentView () == null )
                  || ( orig.getSource () != null && GraphConstants
                      .isDisconnectable ( orig.getSource ().getParentView ()
                          .getAllAttributes () ) ) || ( orig
                  .getSourceParentView () != null && GraphConstants
                  .isDisconnectable ( orig.getSourceParentView ()
                      .getAllAttributes () ) ) );
        if ( target )
          disconnectable = disconnectable
              && ( ( orig.getTarget () == null && orig.getTargetParentView () == null )
                  || ( orig.getTarget () != null && GraphConstants
                      .isDisconnectable ( orig.getTarget ().getParentView ()
                          .getAllAttributes () ) ) || ( orig
                  .getTargetParentView () != null && GraphConstants
                  .isDisconnectable ( orig.getTargetParentView ()
                      .getAllAttributes () ) ) );
            ...
            && disconnectable )
        {
          ...
          boolean acceptSource = source
              && ( graph.getModel ().acceptsSource ( edge.getCell (), null ) || graph
                  .isPreviewInvalidNullPorts () );
          boolean acceptTarget = target
              && ( graph.getModel ().acceptsTarget ( edge.getCell (), null ) || graph
                  .isPreviewInvalidNullPorts () );
          if ( acceptSource || acceptTarget || ! ( source || target ) )
          {
            ...
            {
              dirty.add ( edge.getSource ().getParentView ().getBounds () );
            }
            if ( edge.getTarget () != null )
            {
              dirty.add ( edge.getTarget ().getParentView ().getBounds () );
            }
            if ( graph.isXorEnabled () )
            {
              overlay ( graph.getGraphics () );
            }
            p = graph.fromScreen ( graph
                .snap ( new Point ( event.getPoint () ) ) );
            ...
            p
                .setLocation ( Math.max ( 0, p.getX () ), Math.max ( 0, p
                    .getY () ) );
    ...
    protected Point2D getRelativeLabelPosition ( EdgeView edge, Point2D p )
    {
      ...
      Point2D result = new Point2D.Double (
          ( ( ( ( totalLength / 2 - length - tmp ) / totalLength ) * -2 ) + 1 )
    ...
    public void mouseReleased ( MouseEvent e )
    {
          ...
          && model.acceptsTarget ( edge.getCell (), target ) )
      {
        ...
        if ( clone && initialLabelLocation != null )
        {
          ...
          {
            location = ( Point2D ) edge.getExtraLabelPosition ( currentLabel )
                .clone ();
        ...
        if ( controlPoints != currentPoints )
        {
          ...
          controlPoints.set ( controlPoints.size () - 1, edge.getPoint ( edge
              .getPointCount () - 1 ) );
        }

        if ( clone )
        {
          Map cellMap = graph.cloneCells ( graph.getDescendants ( new Object []
          { edge.getCell () } ) );
      ...
      else
      {
        ...
        edge.refresh ( graph.getGraphLayoutCache (), graph
    ...
    protected void invalidate ()
    {
      ...
      for ( int i = 0 ; i < r.length ; i++ )
      {
        ...
        {
          Point2D s = graph
              .toScreen ( new Point2D.Double ( d.width, d.height ) );
          loc.setFrame ( p.getX () - s.getX () / 2, p.getY () - s.getY () / 2,
              s.getX (), s.getY () );
        }
      }
          ...
          if ( p != null )
          {
            ...
            if ( d != null )
            {
              Point2D s = graph.toScreen ( new Point2D.Double ( d.width,
                  d.height ) );
              extraLabelLocations [ i ].setFrame ( p.getX () - s.getX () / 2, p
                  .getY ()
  ...
  public Point2D getPerimeterPoint ( EdgeView edge, Point2D source, Point2D p )
  {
    ...
    return new Point2D.Double ( ( pe.getX () + p0.getX () ) / 2,
        ( pe.getY () + p0.getY () ) / 2 );
  }
}
