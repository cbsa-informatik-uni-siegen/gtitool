/*
 * @(#)GraphContext.java 1.0 03-JUL-04 Copyright (c) 2001-2004 Gaudenz Alder
 */
 ...
 * of cells.
 * <p>
 * <strong>Note:</strong>The views are consistent subgraphs. Consequently, if
 * an edge points to a selected and an unselected port, the selected port is
 * replaced by its temporary view, but the unselected port is not.
 * 
 * @version 1.0 1/1/02
...
public class GraphContext implements CellMapper
{
  ...
  /**
   * Switch to enable the preview of edge groups, that is, edges that 1 or more
   * children, as part of the context cells.
   */
   ...
   * is contained in this object and visible in the original graph.
   */
    ...
    do
    {
      ...
      while ( it.hasNext () )
      {
        ...
        if ( !cellSet.contains ( obj ) && graphLayoutCache.isVisible ( obj )
            && edge != null && ( PREVIEW_EDGE_GROUPS || edge.isLeaf () ) )
        {
  ...
  public CellView createMapping ( Object cell )
  {
    ...
    view.refresh ( graph.getGraphLayoutCache (), this, true ); // Create
    // Dependent
    // Views
    ...
    if ( src != null )
    {
      view.changeAttributes ( graphLayoutCache, ( AttributeMap ) src
          .getAttributes ().clone () );
  ...
  /**
   * Disconnects the edges in <code>cells</code> from the sources and targets
   * that are not in this context and returns a ConnectionSet that defines the
   * disconnection.
   */
  public ConnectionSet disconnect ( CellView [] cells )
  {
    ...
    for ( int i = 0 ; i < cells.length ; i++ )
    {
      if ( cells [ i ] instanceof EdgeView )
      {
        ...
        {
          if ( port != null
              && GraphConstants.isDisconnectable ( port.getParentView ()
                  .getAllAttributes () ) && !contains ( port.getCell () ) )
          {
            view.setSource ( null );
