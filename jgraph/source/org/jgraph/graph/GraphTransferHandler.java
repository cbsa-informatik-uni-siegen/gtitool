/*
 * @(#)GraphTransferHandler.java 1.0 31-DEC-04 Copyright (c) 2001-2004 Gaudenz
 * Alder
 */
...
public class GraphTransferHandler extends TransferHandler
{
  ...
  protected GraphTransferable create ( JGraph graph, Object [] cells,
      Map viewAttributes, Rectangle2D bounds, ConnectionSet cs, ParentMap pm )
  {
            ...
            && graph.isEnabled () )
        {
              ...
              || graph.getUI ().getDropAction () == TransferHandler.COPY )
          {
            ...
            if ( nested != null )
            {
              ...
              {
                Point2D insert = graph.fromScreen ( graph.snap ( ( Point2D ) p
                    .clone () ) );
              ...
              {
                Point2D insertPoint = getInsertionOffset ( graph, inCount,
                    bounds );
                if ( insertPoint != null )
                {
                  dx = insertPoint.getX ();
          ...
          else
          {
            ...
            if ( p != null )
            {
              ...
              Point2D insert = graph
                  .fromScreen ( graph.snap ( new Point ( p ) ) );
              ...
              else if ( bounds == null )
              {
                ...
                for ( int i = 0 ; i < cells.length ; i++ )
                {
                      ...
                      && !model.isPort ( cells [ i ] ) )
                  {
                    ...
                    if ( tmp == null )
                      tmp = GraphConstants.getBounds ( model
                            ...
                            model.getParent ( cells [ i ] ) ) )
                    {
                      ...
                      if ( view != null && !view.isLeaf () )
                      {
                        ...
                        GraphLayoutCache.translateViews ( new CellView []
                        { view }, dx, dy );
                      }
                      else
                      {
            ...
            graph.setSelectionCells ( DefaultGraphModel.getTopmostCells (
                model, cells ) );
