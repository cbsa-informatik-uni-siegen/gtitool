/*
 * @(#)ParentMap.java 1.0 03-JUL-04 Copyright (c) 2001-2004 Gaudenz Alder
 */
...
public class ParentMap implements Serializable
{
   ...
   * parent of that node.
   * <p>
   * <strong>Note:</strong> Consequently, cells "move up" one level when their
   * parent is removed. <strong>Note:</strong> onlyUsePassedInCells can be used
   * to indicate if only cells from the passed-in cell array are allowed
   * parents. This is only used if remove is not true.
   */
   ...
   * and parent are added to the set of changed nodes. Note: The previous parent
   * is changed on execution of this parent map and must be added by the
   * GraphModel and reflected by the GraphChange.getChanged method. TODO: In
   * general, the GraphModel should be in charge of computing the set of changed
   * cells.
   */
