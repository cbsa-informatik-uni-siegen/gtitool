/*
 * @(#)PortView.java 1.0 03-JUL-04 Copyright (c) 2001-2004 Gaudenz Alder
 */
...
public class PortView extends AbstractCellView
{
  ...
  /**
   * Controls if port magic should be allowed. Default is true. This is an easy
   * switch to disable port magic for all instances of graphs.
   */
  ...
  /**
   * Constructs a view that holds a reference to the specified cell, anchor and
   * parent vertex.
   * 
  ...
  /**
   * This method ensures a non-null value. If the super method returns null then
   * the last valid parent is returned. Note: If a vertex is removed, all ports
   * will be replaced in connected edges. The ports are replaced by the center
  ...
  public Rectangle2D getBounds ()
  {
    ...
    bounds.setFrame ( bounds.getX () - getPortSize () / 2, bounds.getY ()
        - getPortSize () / 2, getPortSize (), getPortSize () );
    return bounds;
  }
          ...
          if ( nearest != null )
          {
                ...
                && nearest.getX () <= r.getX () + r.getWidth () )
            {
              pos.setLocation ( nearest.getX (), pos.getY () );
                ...
                && nearest.getY () <= r.getY () + r.getHeight () )
            { // vertical
  ...
  /**
   * Subclassers can override this to decide whether or not "port magic" should
   * appear on a given edge. (Port magic means the port tries to make the edge
   * horizontal or vertical if the closest control point lies within the bounds
