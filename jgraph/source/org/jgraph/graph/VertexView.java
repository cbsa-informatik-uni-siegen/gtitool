/*
 * @(#)VertexView.java 1.0 03-JUL-04 Copyright (c) 2001-2004 Gaudenz Alder
 */
...
public class VertexView extends AbstractCellView
{
  ...
  { Cursor.NW_RESIZE_CURSOR, Cursor.N_RESIZE_CURSOR, Cursor.NE_RESIZE_CURSOR,
      Cursor.W_RESIZE_CURSOR, Cursor.E_RESIZE_CURSOR, Cursor.SW_RESIZE_CURSOR,
      Cursor.S_RESIZE_CURSOR, Cursor.SE_RESIZE_CURSOR };


  /** Array that holds the cursors for the different control points. */
  public static transient int [] xCursors = new int []
  { Cursor.W_RESIZE_CURSOR, 0, Cursor.E_RESIZE_CURSOR, Cursor.W_RESIZE_CURSOR,
      Cursor.E_RESIZE_CURSOR, Cursor.W_RESIZE_CURSOR, 0, Cursor.E_RESIZE_CURSOR };


  /** Array that holds the cursors for the different control points. */
  public static transient int [] yCursors = new int []
  { Cursor.N_RESIZE_CURSOR, Cursor.N_RESIZE_CURSOR, Cursor.N_RESIZE_CURSOR, 0,
      0, Cursor.S_RESIZE_CURSOR, Cursor.S_RESIZE_CURSOR, Cursor.S_RESIZE_CURSOR };
  ...
  public static class SizeHandle implements CellHandle, Serializable
  {
    ...
    public void mousePressed ( MouseEvent event )
    {
      ...
      for ( int i = 0 ; i < r.length ; i++ )
      {
        if ( r [ i ].contains ( event.getPoint () ) && cursors [ i ] != 0 )
        {
          ...
          Object [] all = AbstractCellView.getDescendantViews ( new CellView []
          { vertex } );
    ...
    public void mouseDragged ( MouseEvent event )
    {
      ...
      {
        // Reset old Bounds
        CellView [] all = AbstractCellView.getDescendantViews ( new CellView []
        { vertex } );
        for ( int i = 0 ; i < all.length ; i++ )
        {
          ...
          if ( orig != null )
          {
            AttributeMap origAttr = ( AttributeMap ) orig.getAllAttributes ()
                .clone ();
            all [ i ]
                .changeAttributes ( graph.getGraphLayoutCache (), origAttr );
