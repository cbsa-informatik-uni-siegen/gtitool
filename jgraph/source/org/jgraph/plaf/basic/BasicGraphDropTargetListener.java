/*
 * @(#)BasicGraphDropTargetListener 1.0 03-JUL-04 Copyright (c) 2001-2004
 * Gaudenz Alder
 */
    ...
    UIResource, ActionListener
{
  ...
  /**
   * Update the geometry of the autoscroll region. The geometry is maintained as
   * a pair of rectangles. The region can cause a scroll if the pointer sits
   * inside it for the duration of the timer. The region that causes the timer
   ...
   * any). If the Scrollable is scrollable along an axis, the step increment is
   * used as the autoscroll inset. If the component is not scrollable, the
  ...
  void updateAutoscrollRegion ( JComponent c )
  {
    ...
    if ( c instanceof Scrollable )
    {
      ...
      i.top = s.getScrollableUnitIncrement ( visible, SwingConstants.VERTICAL,
          1 );
   ...
   * increment of the Scrollable using scrollRectToVisible. If the cursor is in
   * a corner of the autoscroll region, more than one axis will scroll.
   */
  ...
  /**
   * The current component. The value is cached from the drop events and used by
   * the timer. When a drag exits or a drop occurs, this value is cleared.
   */
