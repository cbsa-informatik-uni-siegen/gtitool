/*
 * @(#)BasicGraphUI.java 1.0 03-JUL-04 Copyright (c) 2001-2004 Gaudenz Alder
 */
...
public class BasicGraphUI extends GraphUI implements Serializable
{
   ...
   * during a drag operation. If snap-to-grid mode is disabled, views are moved
   * by a snap increment.
   */
  ...
  public Dimension2D getPreferredSize ( JGraph graph, CellView view )
  {
    ...
    if ( view != null )
    {
      ...
      if ( label || icon )
      {
        ...
        Component component = view.getRendererComponent ( graph, focus, false,
            false );
        if ( component != null )
        {
          graph.add ( component );
  ...
  /**
   * Sets the current location for Drag-and-Drop activity. Should be set to null
   * after a drop. Used from within DropTargetListener.
   */
  ...
  /**
   * Selects the cell and tries to edit it. Editing will fail if the CellEditor
   * won't allow it for the selected item.
   */
  ...
  /**
   * Invoked after the <code>graph</code> instance variable has been set, but
  ...
  /**
   * Invoked as part from the boilerplate install block. This sets the look and
   * feel specific variables in JGraph.
   */
  protected void installDefaults ()
  {
    ...
    graph
        .setHandleColor ( UIManager.getColor ( "MenuItem.selectionBackground" ) );
  ...
  /**
   * Draws the graph to the specified graphics object within the specified clip
   * bounds, if any
   * 
   * @param g the graphics object to draw the graph to
   * @param clipBounds the bounds within graph cells must intersect to be
   *          redrawn
   */
  public void drawGraph ( Graphics g, Rectangle2D clipBounds )
  {
    ...
    if ( clipBounds != null )
    {
      Rectangle2D paintBounds = new Rectangle2D.Double ( clipBounds.getX (),
          clipBounds.getY (), clipBounds.getWidth (), clipBounds.getHeight () );
      realClipBounds = graph.fromScreen ( new Rectangle2D.Double ( paintBounds
          .getX (), paintBounds.getY (), paintBounds.getWidth (), paintBounds
          .getHeight () ) );
    }
      ...
      boolean preview )
  {
    ...
    if ( view != null && bounds != null )
    {
      ...
      Component component = view.getRendererComponent ( graph, sel, bfocus,
          preview );
      rendererPane.paintComponent ( g, component, graph,
          ( int ) bounds.getX (), ( int ) bounds.getY (), ( int ) bounds
              .getWidth (), ( int ) bounds.getHeight (), true );
    }
    // Then Paint Children
    if ( !view.isLeaf () )
    {
  ...
  protected void paintGrid ( double gs, Graphics g, Rectangle2D clipBounds )
  {
    ...
    {
      Rectangle2D graphBounds = graph.getBounds ();
      clipBounds = new Rectangle2D.Double ( 0, 0, graphBounds.getWidth (),
    ...
    if ( ports != null )
    {
      ...
      for ( int i = 0 ; i < ports.length ; i++ )
      {
        if ( ports [ i ] != null )
        {
          ...
          Point2D center = new Point2D.Double ( bounds.getCenterX (), bounds
              .getCenterY () );
          if ( !graph.isPortsScaled () )
            center = graph.toScreen ( center );
          bounds.setFrame ( center.getX () - bounds.getWidth () / 2, center
              .getY ()
  ...
  protected void updateCachedPreferredSize ()
  {
    ...
    preferredSize = new Dimension ( ( int ) Math.max ( min.getX (), scaled
        .getX () ), ( int ) Math.max ( min.getY (), scaled.getY () ) );
    ...
    if ( image != null )
    {
      ...
      preferredSize = new Dimension ( ( int ) Math.max ( preferredSize
          .getWidth (), imageSize.getX () ), ( int ) Math.max ( preferredSize
          .getHeight (), imageSize.getY () ) );
    }
   ...
   * the current editor. Then completeEditing will be messaged with false, true,
   * false to cancel any lingering editing.
   */
    ...
    if ( graph.isCellEditable ( cell ) )
    {
      ...
      if ( cellEditor.isCellEditable ( event ) )
      {
        ...
        Point2D p = getEditorLocation ( cell, editorSize, graph
            .toScreen ( new Point2D.Double ( cellBounds.getX (), cellBounds
                .getY () ) ) );
        ...
        {
          /*
           * Find the component that will get forwarded all the mouse events
           * until mouseReleased.
           */
  ...
  /**
   * Subclassers may override this to provide a better location for the in-place
   * editing of edges (if you do not inherit from the EdgeRenderer class).
   */
      ...
      Point2D pt )
  {
    ...
    if ( view instanceof EdgeView )
    {
      ...
      if ( renderer instanceof EdgeRenderer )
      {
        ...
        pt.setLocation (
            Math.max ( 0, pt.getX () - editorSize.getWidth () / 2 ), Math.max (
                0, pt.getY () - editorSize.getHeight () / 2 ) );
      }
      graph.toScreen ( pt );
    }
    return pt;
    ...
    public void graphChanged ( GraphModelEvent e )
    {
      ...
      if ( inserted != null && inserted.length > 0 )
      {
        ...
        for ( int i = 0 ; i < inserted.length ; i++ )
          graph.updateAutoSize ( graphLayoutCache.getMapping ( inserted [ i ],
              false ) );
      }
      // Change (update size)
      if ( changed != null && changed.length > 0 )
      {
        for ( int i = 0 ; i < changed.length ; i++ )
          graph.updateAutoSize ( graphLayoutCache.getMapping ( changed [ i ],
              false ) );
      }
      ...
      Serializable
  {
     ...
     * Stops editing, updates handles and displays the changed cells.
     */
      ...
      if ( cells != null && cells.length <= MAXCLIPCELLS )
      {
        ...
        if ( r != null )
        {
          ...
          graph.repaint ( ( int ) r.getX () - hsize, ( int ) r.getY () - hsize,
              ( int ) r.getWidth () + 2 * hsize, ( int ) r.getHeight () + 2
                  * hsize );
        }
      }
      else
  ...
  /**
   * Listener responsible for getting cell editing events and updating the graph
   * accordingly.
   */
      ...
      if ( !e.isConsumed () && graph.isEnabled () )
      {
        ...
        if ( !isForceMarquee )
        {
          if ( e.getClickCount () == graph.getEditClickCount ()
              && focus != null && isEditable && focus.getParentView () == null
    ...
    public RootHandle ( GraphContext ctx )
    {
      ...
      if ( !ctx.isEmpty () )
      {
        ...
        if ( initialLocation == null )
          initialLocation = new Point2D.Double ( tmpBounds.getX (), tmpBounds
              .getY () );
        // Sub-Handles
    ...
    public void overlay ( Graphics g )
    {
      ...
      { // Paint Cached Bounds
        ...
        g.drawRect ( ( int ) cachedBounds.getX (),
            ( int ) cachedBounds.getY (), ( int ) cachedBounds.getWidth () - 2,
      ...
      else
      {
        ...
        if ( contextViews != null && isContextVisible )
        {
          ...
          {
            paintCell ( g, contextViews [ i ], contextViews [ i ].getBounds (),
                true );
          }
        }
        if ( !graph.isPortsScaled () )
      ...
      if ( !event.isConsumed () && graph.isMoveable () )
      {
        ...
        { // Start Move if over cell
          ...
          if ( view != null )
          {
            ...
            {
              Rectangle2D bounds = view.getBounds ();
              start = graph.toScreen ( new Point2D.Double ( bounds.getX (),
                  bounds.getY () ) );
      ...
      if ( event != null && !event.isConsumed () )
      {
        ...
        { // Move Cells
          ...
          Point2D point = new Point2D.Double (
              ep.getX () - _mouseToViewDelta_x, ep.getY ()
                  - _mouseToViewDelta_y );
          ...
          if ( isMoving || Math.abs ( dx ) > thresh || Math.abs ( dy ) > thresh )
          {
            ...
            Point2D constrainedPosition = constrainDrag ( event, totDx, totDy,
                dx, dy );
            ...
            if ( dx != 0 || dy != 0 )
            {
              ...
              if ( graph.isAutoResizeGraph ()
                  && ( event.getX () > graph.getWidth () - SCROLLBORDER || event
                      .getY () > graph.getHeight () - SCROLLBORDER ) )
              {
              ...
              if ( targetGroup != null )
              {
                Rectangle2D tmp = graph.toScreen ( ( Rectangle2D ) targetGroup
                    .getBounds ().clone () );
                if ( dirty != null )
                  dirty.add ( tmp );
                else
                      ...
                      .intersects ( AbstractCellView.getBounds ( views ) ) ) )
              {
                targetGroup = ( event.isControlDown () ) ? null
                    : findUnselectedInnermostGroup ( snapCurrent.getX ()
                        / scale, snapCurrent.getY () / scale );
                if ( targetGroup == ignoreTargetGroup )
                  targetGroup = null;
              }
              if ( !snapCurrent.equals ( snapStart )
                  && ( offgraphics != null || !blockPaint ) && !spread )
              {
                if ( graph.isXorEnabled () )
                {
                  overlay ( g );
              ...
              if ( constrained )
                last = ( Point2D ) start.clone ();
              last.setLocation ( last.getX () + dx * scale, last.getY () + dy
                  * scale );
              ...
              if ( overlayed
                  && ( offgraphics != null || !graph.isXorEnabled () ) )
              {
                if ( dirty == null )
                {
                ...
                dirty.setFrame ( dirty.getX () - border2, dirty.getY ()
                    - border2, dirty.getWidth () + border, dirty.getHeight ()
                    + border );
        ...
        if ( event != null && !event.isConsumed () )
        {
          ...
          else if ( isMoving && !event.getPoint ().equals ( start ) )
          {
            if ( cachedBounds != null )
            {
              ...
              GraphLayoutCache
                  .translateViews ( views, tmp.getX (), tmp.getY () );
            }
            CellView [] all = graphLayoutCache.getAllDescendants ( views );
            ...
            { // Move Cells
              ...
              if ( targetGroup != null )
              {
                pm = new ParentMap ( context.getCells (), targetGroup
                    .getCell () );
              }
              ...
              graph.getGraphLayoutCache ().edit ( attributes, disconnect, pm,
                  null );
            }
            event.consume ();
          }
  ...
  /**
   * PropertyChangeListener for the graph. Updates the appropriate variable and
   * takes the appropriate actions, based on what changes.
   */
    ...
    protected void updateInsertionLocation ( JComponent comp, Point p )
    {
      ...
      if ( handle != null )
      {
        ...
        int mod = ( dropAction == TransferHandler.COPY ) ? InputEvent.CTRL_MASK
            : 0;
        handle.mouseDragged ( new MouseEvent ( comp, 0, 0, mod, p.x, p.y, 1,
            false ) );
      }
    }
  ...
  /**
   * @return true if snapSelectedView mode is enabled during the drag operation.
   *         If it is enabled, the view, that is returned by the
   *         findViewForPoint(Point pt), will be snapped to the grid lines. <br>
   *         By default, findViewForPoint() returns the first view from the
   *         GraphContext whose bounds intersect with snap proximity of a mouse
   *         pointer. If snap-to-grid mode is disabled, views are moved by a
   *         snap increment.
   */
   ...
   * 
   * @param snapSelectedView specifies if the snap-to-grid mode should be
   *          applied during a drag operation. If it is enabled, the view, that
   *          is returned by the findViewForPoint(Point pt), will be snapped to
   *          the grid lines. <br>
   *          By default, findViewForPoint() returns the first view from the
   *          GraphContext whose bounds intersect with snap proximity of a mouse
   *          pointer. If snap-to-grid mode is disabled, views are moved by a
