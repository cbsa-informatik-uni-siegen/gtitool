/*
 * Copyright (c) 2005 David Benson See LICENSE file in distribution for
 * licensing details of this source file
 */
 ...
 * cubic bezier curve. Let p a list of given points and b the calculated bezier
 * points, then one get the whole curve by: sharedPath.moveTo(p[0])
 * sharedPath.quadTo(b[0].x, b[0].getY(), p[1].x, p[1].getY()); for(int i = 2; i <
 * p.length - 1; i++ ) { Point b0 = b[2*i-3]; Point b1 = b[2*i-2];
