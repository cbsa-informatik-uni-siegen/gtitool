...
public class ParallelEdgeRouter extends LoopRouting
{
  ...
  public List routeEdge ( GraphLayoutCache cache, EdgeView edge )
  {
    ...
    // if (graph == null) {
    // System.out
    // .println("graph variable not correctly set, must be set to obtain
    // parallel routing");
    // }
    ...
    if ( edges.length >= 2 )
    {
      ...
      Point2D from;
      Point2D perimeterPoint = edge.getTarget () != null ? edge.getPoint ( edge
          .getPointCount () - 1 ) : AbstractCellView.getCenterPoint ( nodeTo );
          ...
          || edge.getSource ().getParentView () == null )
      {
        ...
        from = nodeFrom.getPerimeterPoint ( edge, AbstractCellView
            .getCenterPoint ( nodeFrom ), perimeterPoint );
      }
      else
      {
        from = edge.getSource ().getParentView ()
            .getPerimeterPoint (
                edge,
                AbstractCellView.getCenterPoint ( edge.getSource ()
                    .getParentView () ),
                ( edge.getTarget () != null && edge.getTarget ()
                    .getParentView () != null ) ? AbstractCellView
                    .getCenterPoint ( edge.getTarget ().getParentView () )
      ...
      { // INV: nodeTo != null
        ...
        to = nodeTo.getPerimeterPoint ( edge, AbstractCellView
            .getCenterPoint ( nodeTo ), from );
      }
      else
      {
        to = edge.getTarget ().getParentView ().getPerimeterPoint (
            edge,
            AbstractCellView.getCenterPoint ( edge.getTarget ()
                .getParentView () ), from );
      }
      ...
      if ( from != null && to != null )
      {
        ...
        else
        {
          ...
          Point2D controlPointFrom = new Point2D.Double ( ex + midXFrom, ey
              + midYFrom );
            ...
            if ( allAttributes != null )
            {
                  ...
                  || lineStyle == GraphConstants.STYLE_SPLINE )
              {
                ...
                GraphConstants.setLabelPosition ( allAttributes,
                    new Point2D.Double ( x, r * edgeMiddleDeparture / 79 ) );
              }
              else
              {
      ...
      CellView cellView1, CellView cellView2 )
  {
    ...
    while ( iter1.hasNext () )
    {
      ...
      while ( iter2.hasNext () )
      {
        ...
        Object [] edges = DefaultGraphModel.getEdgesBetween ( model, tempCell1,
            tempCell2, false );
        if ( edges.length > 0 )
        {
          // for (int i = 0; i < edges.length; i++) {
          ...
          else
          {
            ...
            System.arraycopy ( edgesBetween, 0, newArray, 0,
                edgesBetween.length );
            System.arraycopy ( edges, 0, newArray, edgesBetween.length,
                edges.length );
