...
erkennt man an einem Pfeil, der optional mit Start beschriftet sein
kann.\vspace{10pt}
...
Um dies zu realisieren, haben wir uns die frei verfügbare Java Bibliothek JGraph
...
entschieden, die Kernelemente als Attribut in den graphischen Komponenten
...
beinhaltet diese Anpassung nicht allzu viele Verbesserungen. Das sieht bei
der Verwendung von mehreren Symbolen in einem Übergang oder den
Potenzmengen-Zuständen aber anders aus. Dort ist der Benutzer in der Lage, durch
die zusätzlichen Farbinformation und Zeichenattribute, die ihm zur Verfügung
...
dass in der Literatur, sowie im Vorlesungsskript, Potenzautomaten-Zustand nicht
...
Umsetzungen diskutiert. Zur Diskusion standen unter anderen, dass in jeder Zelle
der Tabelle ausgewählt werden kann, zu welchen Zuständen ein Übergang existiert.
Diese Auswahl hätte allerdings bei der Umsetzung schlecht ausgesehen, zumindest
bei hinreichend vielen Zuständen. Aus diesem Grund wurde dieser Ansatz
...
in jede Zelle einen Parser zu verwenden. Dieser Parser müsste so implementiert
...
Als Orientierungshilfe für den Benutzer kann man einen Eintrag in der Tabelle
...
aktiven Zuständen angesprochen, was dem Benutzer erleichtern soll, zu
...
konstruierte Automat ein beliebiges Eingabewort abarbeitet, dabei muss
allerdings gelten, dass es sich um ein gültiges Wort handelt, es müssen also
alle Symbole im Alphabet vorkommen. Für diesen Zweck wurde die Wortnavigation
entwickelt.\vspace{10pt}
